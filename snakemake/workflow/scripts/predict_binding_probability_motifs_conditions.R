...
# Predict binding probability for multiple TF motifs across conditions
library(optparse)
library(TOP)
library(data.table)
...
for( i in 1:nrow(metadata)) {
  ...
  if(length(sample_ids) == 1){
    ...
    if(file.exists(data_file) && file.size(data_file) > 0){
      ...
      outfile <- paste0(tf_name, '_', pwm_id, '_', thresh_pValue, '.', cell_type, '.', condition, '.', model_level, '.level.predicted.binding.probability.txt.gz')
