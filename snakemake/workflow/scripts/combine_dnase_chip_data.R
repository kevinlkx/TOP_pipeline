...
# Combine DNase and ChIP data for a TF x cell type combo
library(optparse)
suppressPackageStartupMessages(library(data.table))
source('../scripts/normalize_bin_utils.R')
...
# This function takes normalized DNase and ChIP data with replicates merged.
combine_dnase_bins_chip_data <- function(dnase_counts_file, chip_counts_file){

  if ( !file.exists(dnase_counts_file) || (file.size(dnase_counts_file) == 0) ) {
    message('DNase file does not exist or is empty. \n')

  } else if (!file.exists(chip_counts_file) || (file.size(chip_counts_file) == 0) ) {
    message('ChIP file does not exist or is empty. \n')
    ...
    normalized_dnase_counts.df <- readRDS(dnase_counts_file)

    if ( any(sites.df[,2] != normalized_dnase_counts.df[,2]) ){
      stop('Sites do not match!')
    }

    dnase_bins <- bin_dnase(as.matrix(normalized_dnase_counts.df[,-c(1:4)]), bin_method, transform)

    combined_data <- data.frame(sites.df, dnase = dnase_bins, chip = chip_counts.df$chip_normalized)
    colnames(combined_data) <- c(colnames(sites.df), paste('dnase', colnames(dnase_bins), sep = '.'), 'chip')
...
if( !file.exists(chip_counts_file) || file.size(chip_counts_file) == 0 ){
  ...
  if ( length(dnase_samples) == 1 ) {
    ...
    dnase_counts_file <- paste0(dnase_countmatrix_dir,'/', dnase_samples, '/', pwm_id, '_', thresh_pValue, '.normalized_countmatrix.rds')
    combined_data <- combine_dnase_bins_chip_data(dnase_counts_file, chip_counts_file)
    ...
    for (i in 1:length(dnase_samples)) {
      ...
      dnase_counts_file <- paste0(dnase_countmatrix_dir,'/', dnase_samples[i], '/', pwm_id, '_', thresh_pValue, '.normalized_countmatrix.rds')
      combined_data <- combine_dnase_bins_chip_data(dnase_counts_file, chip_counts_file)
    ...
    merged_dnase_counts_file <- paste0(dnase_countmatrix_dir,'/merged_reps/', pwm_id, '_', thresh_pValue, '.', cell_type, '.merged_normalized_countmatrix.rds')
    saveRDS(merged_dnase_data, merged_dnase_counts_file)

    combined_data <- combine_dnase_bins_chip_data(merged_dnase_counts_file, chip_counts_file)
